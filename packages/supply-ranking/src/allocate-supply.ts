...
export type AllocationRefusalReason =
  | RankingRefusalReason
  | 'unreadable_amount'
  | 'ambiguous_supplier_stock';

export type SupplyAllocationResult =
  | Readonly<{ kind: 'allocated'; allocation: SupplyAllocation }>
  | Readonly<{ kind: 'refused'; reason: AllocationRefusalReason; detail: string }>;
 ...
 *
 * Splitting rests on one assumption the feed does not state: that each offer's
 * availability is stock nobody else's availability already counts. Across two
 * suppliers that holds. Across two offers from the same supplier it does not,
 * so drawing on one supplier twice is refused rather than summed.
 */
export function allocateAcrossSuppliers(
  ...
  for (const entry of ranking.ranking.ranked) {
    ...
    /**
     * Two offers from one supplier for one product may be two stock pools or
     * one pool described twice, and nothing in the feed distinguishes them.
     * Adding them is a guess that fails in the direction that hurts: an order
     * placed for stock that was never there. Refusing names the ambiguity for
     * whoever can resolve it, where a short fill would blame the supplier for
     * a shortage the allocator invented.
     */
    if (drawnFrom.has(entry.supplierId)) {
      return { kind: 'refused', reason: 'ambiguous_supplier_stock', detail: entry.supplierId };
    }
    ...
    drawnFrom.add(entry.supplierId);
