 ...
 *
 * Every mode ends `supplier_id, offer_id`. Supplier identity alone cannot
 * separate two offers held by one supplier, so a pair tied through it compared
 * equal and their order was whatever the feed happened to emit. Offer identity
 * is unique once duplicates are refused, which makes each precedence a total
 * order and the ranking reproducible from the inputs alone.
 */
export const SORT_CRITERIA: Readonly<Record<
  Exclude<SortMode, 'recommended'>,
  readonly RankingCriterion[]
>> = {
  price: ['line_total', 'lead_time_days', 'fulfilment_rate', 'supplier_id', 'offer_id'],
  lead_time: ['lead_time_days', 'line_total', 'fulfilment_rate', 'supplier_id', 'offer_id'],
  rating: ['fulfilment_rate', 'line_total', 'lead_time_days', 'supplier_id', 'offer_id'],
  available_quantity: [
    'available_quantity', 'line_total', 'lead_time_days', 'supplier_id', 'offer_id',
  ],
};
...
export function rankEligibleSupply(
  ...
  const repeatedOffer = firstRepeat(offers.map((entry) => entry.offerId));
  if (repeatedOffer !== undefined) {
    return { kind: 'refused', reason: 'duplicate_offer', detail: repeatedOffer };
  }

  const repeatedStanding = firstRepeat(standings.map((entry) => entry.supplierId));
  if (repeatedStanding !== undefined) {
    return { kind: 'refused', reason: 'duplicate_supplier_standing', detail: repeatedStanding };
  }
...
/**
 * Names the smallest repeated identifier rather than the first one encountered,
 * so a malformed feed is reported identically however it was ordered. Reporting
 * positionally would make two permutations of the same broken input disagree
 * about which record is at fault.
 */
function firstRepeat(ids: readonly string[]): string | undefined {
...
function assessOffer(
  ...
  const minimumSign = compareDecimals(offer.minimumOrderQuantity, '0');
  ...
  if (lineTotal === undefined || stockOrder === undefined || minimumOrder === undefined
    || minimumSign === undefined || stockSign === undefined || minimumAgainstStock === undefined) {
    return 'unreadable_amount';
  }
  /**
   * A minimum below zero is not a supplier without a minimum, it is a field
   * nothing wrote correctly. Read literally it satisfies the gate it exists to
   * enforce, so every later check on it passes and the offer reaches a purchase
   * order on the strength of a value nobody meant. Zero is the honest way to
   * say there is no minimum, and it is still accepted.
   */
  if (minimumSign < 0) {
...
function criterionKey(criterion: RankingCriterion, entry: EligibleOffer): string {
  ...
  if (criterion === 'offer_id') {
    return entry.offer.offerId;
  }
  return entry.offer.supplierId;
}

/**
 * Falls through to offer identity, which every precedence already ends with, so
 * the comparator returns zero only for an offer against itself. Duplicate offer
 * identifiers are refused before this runs, which is what makes that true.
 */
function compareEligibleOffers(
  ...
  return compareIds(left.offer.offerId, right.offer.offerId);
}
...
function compareCriterion(
  ...
  return compareIds(left.offer.supplierId, right.offer.supplierId);
}
