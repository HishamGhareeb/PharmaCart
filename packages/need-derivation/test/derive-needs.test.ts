...
function derived(result: NeedDerivation) {
...
function shortfallFor(
  ...
  const result = derived(deriveNeeds(targets, positions, commitments));
...
describe('need derivation', () => {
  ...
  it('subtracts what is already on order, so nothing is bought twice', () => {
    assert.equal(shortfallFor([position()], [commitment({ quantity: '10' })]), '5');
    assert.equal(
      shortfallFor([position()], [
        commitment({ commitmentId: 'c-1', quantity: '6' }),
        commitment({ commitmentId: 'c-2', quantity: '4' }),
      ]),
      '5',
    );
    assert.equal(
      shortfallFor([position()], [commitment({ sourceCode: 'SKU-OTHER', quantity: '10' })]),
      '15',
    );
  });
  ...
  it('reports what it read, so a need can be explained without recomputing it', () => {
    const result = derived(deriveNeeds([target], [position()], [commitment({ quantity: '3' })]));
  ...
  it('orders its output so two identical inputs produce one answer', () => {
    ...
    const result = derived(deriveNeeds(targets, positions, []));
...
describe('what need derivation refuses to guess', () => {
  ...
  it('withholds rather than converting between units', () => {
    ...
    assert.equal(
      withholdingFor([position()], [commitment({ unit: 'strip' })]),
      'unit_mismatch',
    );
  });
  ...
  it('withholds a position already covered by stock or by open orders', () => {
    ...
    assert.equal(
      withholdingFor([position({ onHand: '5' })], [commitment({ quantity: '15' })]),
      'covered',
    );
  });
  ...
  it('withholds anything it cannot read as an exact quantity', () => {
    ...
    assert.equal(
      withholdingFor([position()], [commitment({ quantity: 'lots' })]),
      'unreadable_amount',
    );
  });
  ...
  it('reports every withheld position rather than only the first', () => {
    ...
    const result = derived(deriveNeeds(targets, [position({ stale: true })], []));
