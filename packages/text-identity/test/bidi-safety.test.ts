...
describe('identifier text safety', () => {
  ...
  it('refuses digits that only look like the digits they are not', () => {
    ...
    assert.equal(identifierRejection('SKU-１２３'), 'non_ascii_digit');
    assert.equal(identifierRejection('SKU-१२३'), 'non_ascii_digit');
  });
  ...
  it('refuses an empty or oversized identifier', () => {
    ...
    assert.equal(identifierRejection(' \t\r\n'), 'empty_identifier');
    assert.equal(identifierRejection('a'.repeat(257)), 'identifier_too_long');
  });

  it('refuses C0 and C1 control characters', () => {
    assert.equal(identifierRejection('SKU\u001f-1'), 'control_character');
    assert.equal(identifierRejection('SKU\u0085-1'), 'control_character');
    assert.equal(identifierRejection('SKU\u009f-1'), 'control_character');
  });
});
