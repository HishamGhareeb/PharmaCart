...
describe('symlink-aware root containment', () => {
  ...
  it('does not read through a symlink that escapes the root', async (t) => {
    const dropRoot = path.join(base, 'drop');
    const link = path.join(dropRoot, 'read-escape.csv');
    try {
      await fs.symlink(path.join(base, 'outside', 'secrets.env'), link);
    } catch {
      t.skip('symlink creation is not permitted in this environment');
      return;
    }

    const decision = await readContainedFileWithinRoot(dropRoot, 'read-escape.csv', 1024);
    assert.deepEqual(decision, { kind: 'rejected', reason: 'escapes_root', detail: 'read-escape.csv' });
  });

  it('stops reading when a file exceeds its byte budget', async () => {
    const dropRoot = path.join(base, 'drop');
    await fs.writeFile(path.join(dropRoot, 'oversize.csv'), '0123456789', 'utf8');

    const decision = await readContainedFileWithinRoot(dropRoot, 'oversize.csv', 8);
    assert.deepEqual(decision, { kind: 'rejected', reason: 'input_too_large', detail: 'oversize.csv' });
  });

  it('rejects a directory instead of treating it as feed bytes', async () => {
    const decision = await readContainedFileWithinRoot(path.join(base, 'drop'), 'branch-01', 1024);
    assert.deepEqual(decision, { kind: 'rejected', reason: 'read_failed', detail: 'branch-01' });
  });
