...
The reader is injected through `ContainedFileReader`. It receives the root, relative path and a required byte ceiling; a conforming reader must resolve links inside the allowlisted root before opening and must stop reading at that ceiling. `containedFilesystemReader` is the filesystem implementation and refuses non-regular file handles. The byte ceiling is trusted service configuration and must not come from feed input. The pipeline validates the ceiling and repeats the returned byte-length check so a faulty injected reader cannot pass an oversized buffer onward. The string path guard still runs before the reader, and a traversal path records zero reader calls. Unexpected reader errors become the stable `read_failed` refusal rather than escaping the ingestion boundary.
...
The pipeline covers the delimited-file transport only. The JSON and database-view readers exist in `packages/transport-adapters` and converge on the same adapter, so composing them is mechanical, but it is not written. The filesystem reader resolves and opens a canonical path, but it does not claim atomic protection against an attacker who can rename directories inside the configured drop root during the check; deployment must keep the root and its directories service-owned, with external writers restricted to controlled drop locations.
