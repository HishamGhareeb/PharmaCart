...
No transport calls this package yet, so AC-013 stays NOT RUN. Unit coverage of a guard is not the same as evidence that hostile input meets the guard before a parser; the criterion moves only when a real ingestion path is wired and the malicious-input corpus runs through it.
...
The guards are pure and independently testable, so they carry no vendor assumptions. They must be called at the transport edge, before any adapter, and their rejection must be terminal for that input rather than a warning.
